...
byte Relay_1 = 12; // подключаем реле к цифровому пину
byte Relay_2 = 13; // подключаем реле к цифровому пину
byte Ground_Hum_first = A2; // подключаем щуп к аналоговому пину
byte Ground_Hum_second = A3; // подключаем щуп к аналоговому пину
byte Water_sens = A4;

//переменные для работы
boolean Test = false; // значение для режима "наблюдателя"
int Crit_CO2 = 900; // значение критического углекислого газа
...
void setup() {
  if (Test == true) {
  Serial.begin(9600); }
  // если "ПРАВДА" то включается серийный порт
...
void loop() {
  ...
  lcd.setCursor(0, 0);
  lcd.print("Started");
  int Gas = CO2(); // опрос датчика углекислого газа
  if (Gas > Crit_CO2) //если текущее значение больше критического 
   { lcd.begin(16,2);
    lcd.setCursor(2,0);
  ...
  if (Ground_Humidity () == 500) {
     digitalWrite(Relay_1, HIGH);   // реле включено
     delay(10000);  
     digitalWrite(Relay_1, LOW);  // реле выключено
     delay(2000);
  }
...
int CO2() //инициализация датчика СО2
{
  int CO2_sens_Value = analogRead(CO2_sens);
  return CO2_sens_Value;
}

int Light ()//инициализация датчика освещения
{
  int photo_sens_value = digitalRead(Photo_sens);
  Serial.println(photo_sens_value);
...
int Ground_Humidity1() //инициализация щупов
{
    int Grnd_Hum1 = analogRead(Ground_Hum_first);
    return Grnd_Hum1;
}
...
int Ground_Humidity() {
  int sum_hum = (Ground_Humidity1() + Ground_Humidity2())/2;
  return sum_hum;
}
...
int Sens_Dht_temp1()//инициализация датчика DHT-11
{
...
int menu_right1 () {
  ...
  while (button == 1) // если нажата кнопка вправо
 {
     ...
     int buttonValue = analogRead(0);
     Serial.println(buttonValue);
     if (buttonValue == 719 || buttonValue == 479 || buttonValue == 307 || buttonValue == 131) break; //если нажаты другие кнопки 
...
int menu_right2 () {
  ...
  while (button == 1) // если нажата кнопка вправо
 {
     ...
     if (buttonValue == 479) menu_right1(); // если снова нажата кпопка влево
...
int menu_down() {
  ...
  while (button == 3) // если нажата кнопка вправо
 {
     ...
     int buttonValue = analogRead(0);
     Serial.println(buttonValue);
     if (buttonValue == 0 || buttonValue == 131 || buttonValue == 307 || buttonValue == 719 || buttonValue == 479) loop(); //если нажаты другие кнопки 
     int Gas = CO2();
     lcd.setCursor(0,1);
     lcd.print("CO2 ");
     lcd.print(Gas);
